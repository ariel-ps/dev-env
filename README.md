...
| `term` | — | `flash-term` (flash the calling terminal's window a color then restore — identifies the emulator + pts by walking process ancestry, so it works even when stdout is a pipe, e.g. under Claude Code), `pflash` (mood-driven combo: flash a mood-matched color **and** play the matching `psound` together, e.g. `pflash done`/`pflash error`), `bin/kitty-agent-title` (agent-lifecycle hook, not a shell function: wired into `~/.claude/settings.json` and `~/.codex/hooks.json` so each pane's title shows which agent owns it, which repo/branch it is in, and its state — `C ps-agent/PROE-7695 ⚙`, `❓` when blocked on a permission prompt, `✔` when done; also publishes the same state as a JSON record under `$CX_STATE_DIR/state/<session_id>.json` so `cx ls` can read status exactly instead of scraping panes; `kitty-tab-title.sh` registers a `precmd` hook that clears the override and restores the cwd once you are back at a prompt) |
